...
- Never create, edit, rotate, or delete anything in 1Password. Ask the user to make the change and only declare the required fields in Git.
...
- I have a webhook to reconcile on Git push, so never manually reconcile unless fixing a specific issue with a HelmRelease or Kustomization
...
- Never suggest using the task commands
- Never run `git add` or `git commit`; always give me a one-line commit message based on the changed files in the format `verb(app): summary`, for example `fix(reptile-tracker): revert timezone changes in scheduler to fix notification crashes`
- Before using `kubectl` or `talosctl`, prefix the command with `export KUBECONFIG=/home/chrto/Homelab/github/chrtol/home-cluster/kubeconfig &&`
- For reusable patterns (auth/OIDC, backup, gatus, PVC provisioning, etc.), create generalized components in `/kubernetes/components/`
- Prefer OCI repositories over Helm repositories for HelmReleases
- Place Markdown documents in `/ai-activity/`; create a topic subfolder when multiple documents are needed
