...
- `VPS_DEADMAN_GRACE_SECONDS` - Seconds without a ping before alerting (600 = 10 min; role default is 600 if unset)
...
> All five fields must exist in `Lab/vps-proxy` — `load-secrets-action` fails the CI run on any
> missing `op://` reference.
